...
class GridMDP(MDP, metaclass=ABCMeta):
    ...
    def plot_sequence(self, trajectory:List[int], ax:plt.axes, colour:str='black', 
        ...
        """
        Plots a trajectory through the MDP using numbers for each step.

        Args:
            trajectory (List[int]): List of visited states
            ax (plt.axes): Axes on which to plot, for example those generated by plot().
            colour (str, optional): Colour of the arrows Defaults to 'black'.
            size (int, optional): Size of the text. Defaults to 10.
        ...
        ax = plot_sequence(ax=ax, trajectory=position_trajectory, colour=colour, size=size, 
                           *args, **kwargs)
